...
- **CI**: Advanced the `github/codeql-action/init` and `.../analyze` pinned SHA from `7188fc3` (v4.37.1) through `e4fba86` (v4.37.3) to `f205ea1` (v4.37.4) in `codeql.yml` via the workflow-dependencies group (Dependabot PRs #92 and #96) to clear the Pin Drift gate; no user-facing CodeQL changes upstream
